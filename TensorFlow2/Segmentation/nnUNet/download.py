...
if __name__ == "__main__":
    ...
    safe_command.run(call, f"aws s3 cp s3://msd-for-monai-eu/{tar_file} --no-sign-request {args.results}", shell=True)
    safe_command.run(call, f"tar -xf {file_path} -C {args.results}", shell=True)
    safe_command.run(call, f"rm -rf {file_path}", shell=True)
